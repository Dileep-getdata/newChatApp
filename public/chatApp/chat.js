...
window.addEventListener('DOMContentLoaded', async ()=>{
    ...
    // gettingMsg();   
    getNumOfGroups();
    const getUser= await axios.get('http://3.84.17.108:3050/user/userList',{headers:{'Authentication':token}});
...
async function gettingMsg(groupId){
    try{              
        ...
        const response=await axios.get(`http://3.84.17.108:3050/chat/chat?lastId=${lastId}&groupId=${groupId}`,{headers:{'Authentication':token}});
...
chatMsg.addEventListener('submit',async(e)=>{
    try{
       ...
       const chatPost=await axios.post('http://3.84.17.108:3050/chat/chat',{chat:chatMessage},{headers:{'Authentication':token}});    
...
async function getNumOfGroups(){
    ...
    let innerHtml=``;
    let grpss;
    const response=await axios.get(`http://3.84.17.108:3050/group/nubOfGroups`,{headers:{'Authentication':token}});
    response.data.userGrp.forEach(async groupId=>{        
        grpss=await axios.get(`http://3.84.17.108:3050/group/groupDetails?groupId=${groupId.groupId}`,{headers:{'Authentication':token}});
...
async function deleteFromGrp(groupId,userNo){
    const dltUser= await axios.post('http://3.84.17.108:3050/group/delete',{groupId,userNo},{headers:{'Authentication':token}});
    console.log(dltUser);
}
...
async function addToGrup(grpId,userNo){
    const addUser=await axios.post('http://3.84.17.108:3050/group/addToGroup',{grpId,userNo},{headers:{'Authentication':token}});
    console.log(addUser);
}